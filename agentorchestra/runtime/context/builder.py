...
class ContextBuilder:
    ...
    def __init__(
        ...
        knowledge_provider: Optional[Callable[[str], List['ContextPacket']]] = None
    ):
        """初始化 ContextBuilder

        Args:
            config: 上下文构建配置
            knowledge_provider: 知识提供者回调，接收用户查询，
                返回额外的 ContextPacket 列表（如从知识图谱检索）。
        """
    ...
    def _gather(
        ...
        # 知识提供者：从知识图谱/对象存储检索相关知识（用户自定义回调）
