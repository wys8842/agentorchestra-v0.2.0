...
class Tool(ABC):
    ...
    def __init__(self, name: str, description: str, expandable: bool = False,
        ...
        Args:
            ...
            read_only: 是否只读工具（不修改外部状态，用于工具过滤）
        """
        self.name = name
    ...
    async def arun(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        默认实现：在线程池中运行同步 run_with_timing() 方法
        子类可以重写此方法实现真正的异步执行
        ...
        return await loop.run_in_executor(
            None,
            lambda: self.run_with_timing(parameters)
        )

    async def arun_with_timing(self, parameters: Dict[str, Any]) -> ToolResponse:
        """异步执行工具并自动添加时间统计

        通过调用 arun() 委托（arun 已内嵌 timing 逻辑），
        这里只负责异常时的错误响应包装。
    ...
    def get_expanded_tools(self) -> Optional[List['Tool']]:
        ...
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            if hasattr(method, '_is_tool_action'):
                tool: Tool = AutoGeneratedTool(
                    parent=self,
                    method=method,
                    name=method._tool_name,  # type: ignore[attr-defined]
                    description=method._tool_description  # type: ignore[attr-defined]
                )
                tools.append(tool)
    ...
    def to_openai_schema(self) -> Dict[str, Any]:
        ...
        for param in parameters:
            ...
            # 如果是数组类型，添加 items 定义
            if param.type == "array":
                prop["items"] = {"type": "string"}  # 默认字符串数组  # type: ignore[literal-required]
